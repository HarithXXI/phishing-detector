...
async def analyze_image_vision(image_bytes: bytes, mime_type: str = "image/png") -> Dict[str, Any]:
    """
    Extract text and analyze phishing indicators from an image using Gemini 1.5 Flash Vision API.
    """
    if not GEMINI_API_KEY:
        return {"extracted_text": "", "error": "GEMINI_API_KEY not configured"}

    b64_img = base64.b64encode(image_bytes).decode("utf-8")
    models_to_try = [GEMINI_MODEL, "gemini-1.5-flash", "gemini-1.5-pro"]

    prompt = (
        "You are an OCR and security vision system. Extract ALL text present in this screenshot/image. "
        "Return ONLY a clean JSON object with this format:\n"
        "{\n"
        '  "extracted_text": "text extracted from image",\n'
        '  "is_phishing": true/false,\n'
        '  "risk_level": "LOW" or "MEDIUM" or "HIGH",\n'
        '  "attack_type": "phishing" or "smishing" or "clean" or "suspicious"\n'
        "}"
    )
            ...
            async with httpx.AsyncClient(timeout=HTTP_TIMEOUT_SEC) as client:
                ...
                if resp.status_code == 200:
                    ...
                    if candidates:
                        ...
                        if isinstance(parsed, dict) and "extracted_text" in parsed:
                            return parsed
                        return {"extracted_text": text.strip(), "is_phishing": False, "risk_level": "LOW"}
        except Exception as exc:
            log.warning("[Gemini Vision] Failed with model %s: %s", model, exc)

    return {"extracted_text": "", "error": "Vision API unavailable"}
